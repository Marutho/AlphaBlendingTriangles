namespace Demo1
{
    ...
    public class SoftwareRasterizerCore : IDisposable
    {
        ...
        public void Draw( Vector3[] vertices, Viewport viewport, ConstantBuffer constantBuffer )
        {
            constantBuffer.color = new Vector3( constantBuffer.color.Z, constantBuffer.color.Y, constantBuffer.color.X );

            Rasterize( vertices, viewport, constantBuffer );

            if ( outputMode == OutputMode.Color )
                CopyToBackbuffer( backbufferBitmap );
            else
                CopyToBackbuffer( depthbufferBitmap );
        }

        virtual protected void Rasterize( Vector3[] vertices, Viewport viewport, ConstantBuffer constantBuffer ) { }

        private void CopyToBackbuffer( Bitmap bitmap )
        {
