namespace Demo1
{
    ...
    public class SoftwareRasterizer : SoftwareRasterizerCore
    {
        ...
        {
            float s = Remap(v1.Y, v3.Y, y);
            float x0 = Lerp(v1.X, v2.X, s); // ?  
            float x1 = Lerp(v1.X, v3.X, s); // ?

            //we need for the drawlinedepthtest, calculate depth0 and depth 1 first
            float depth0 = Lerp(v1.Z, v3.Z, s); // ?  
            float depth1 = Lerp(v1.Z, v3.Z, s); // ?

            DrawLineDepthTest( y, x0, x1, depth0, depth1, color );
        }
        ...
        private void DrawLineDepthTest( float y, float x0, float x1, float depth0, float depth1, Vector3 color )
        {
            ...
            for ( float x = (float) Round( t0 ) + 0.5f; x < (float) Round( t1 ) + 0.5f; x++ )
            {
                ...
                float depthPosition = (int)(255 * Min(depth, 1.0f));
                // ?
                if ((int)(255 * Min(depth, 1.0f)) < depthbufferBitmap.GetPixel((int)x, (int)y).B)
                {
