...
namespace Demo1
{
    ...
    public class Renderer : IDisposable
    {
        ...
        private D3D11.Buffer triangleColorsVertexBuffer;
        ...
        Vector3[] vertexPositions = new Vector3[] { new Vector3( 0.5f, -0.5f, 1.0f ), new Vector3( -0.5f, -0.25f, 1.0f ), new Vector3( 0.0f, 0.5f, 0.0f ) };
        Vector3[] vertexColors = new Vector3[] { new Vector3(1.0f, 0.0f, 0.0f), new Vector3(0.0f, 1.0f, 0.0f), new Vector3(0.0f, 0.0f, 1.0f) };
        #else
        Vector3[] vertexPositions = new Vector3[] { new Vector3( 0.5f, -0.5f, 0.0f ), new Vector3( -0.5f, -0.25f, 0.0f ), new Vector3( 0.0f, 0.5f, 0.0f ) };
        Vector3[] vertexColors = new Vector3[] { new Vector3(1.0f, 0.0f, 0.0f), new Vector3(0.0f, 1.0f, 0.0f), new Vector3(0.0f, 0.0f, 1.0f) };
        #endif
        ...
        private void InitializeTriangle()
        {
            ...
            triangleColorsVertexBuffer = D3D11.Buffer.Create<Vector3>(device, D3D11.BindFlags.VertexBuffer, vertexColors);
        }
        ...
        public void Dispose()
        {
            ...
            triangleColorsVertexBuffer.Dispose();
        ...
        public void Render()
        {
            ...
            if ( mode == RenderMode.RenderModeHardware )
            {
                ...
                deviceContext.InputAssembler.SetVertexBuffers( 1, new D3D11.VertexBufferBinding(triangleColorsVertexBuffer  , elementSize, 0));
            ...
            if ( mode == RenderMode.RenderModeHardware )
            {
                UpdateConstantBuffer( offsetScale, Vector3.One );
                deviceContext.Draw( vertexCount, 0 );
            }
            else
            {
                SoftwareRasterizer.ConstantBuffer softwareRasterizerConstantBuffer = new SoftwareRasterizer.ConstantBuffer();
                softwareRasterizerConstantBuffer.offsetScale = offsetScale;
                softwareRasterizerConstantBuffer.color = Vector3.One;
                softwareRasterizer.Draw( vertexPositions, viewport, softwareRasterizerConstantBuffer );
            }
            #endif
        }
